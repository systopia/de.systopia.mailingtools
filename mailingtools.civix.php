...
/**
 * @return CRM_Mailingtools_Upgrader
 */
function _mailingtools_civix_upgrader() {
  if (!file_exists(__DIR__ . '/CRM/Mailingtools/Upgrader.php')) {
    return NULL;
  }
  else {
    return CRM_Mailingtools_Upgrader_Base::instance();
  }
}
 ...
 * missing them.
 */
function _mailingtools_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _mailingtools_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _mailingtools_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _mailingtools_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
